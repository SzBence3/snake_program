...
float getFacingDir(Api* api){ // prob needs to be negatived
...
const float distance_limit=1000;
food getFoodTarget_L(Api *api){
...
string to_string(food f){
    ...
    return "x: "+ to_string(x) + " y: "+ to_string(y) + " value: " + to_string(f.val);
}
